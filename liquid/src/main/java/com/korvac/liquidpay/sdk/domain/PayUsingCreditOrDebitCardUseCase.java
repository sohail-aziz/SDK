...
public class PayUsingCreditOrDebitCardUseCase extends AuthenticatedUseCase<PayUsingCreditOrDebitCardUseCase.Params> {

    LiquidPayPrefs liquidPayPrefs;
    Repository repository;

    @Inject
    public PayUsingCreditOrDebitCardUseCase(LiquidPayPrefs liquidPayPrefs, Repository repository) {
    ...
    void callRealAPI(LiquidPayConfig config,String accessToken, final Params params) {
        ...
        repository.payUsingCreditOrDebitCard(config, accessToken, params.paymentId, params.source, new LiquidPayCallback<ResponsePayment>() {
            @Override
